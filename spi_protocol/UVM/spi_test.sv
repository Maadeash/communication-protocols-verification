class spi_base_test extends uvm_test;
  ...
  task wait_time_budget();
    ...
    if(clks<=0) begin
      `uvm_warning("TEST",$sformatf("sim_time_ns=%0d / clk_period_ns=%0d gave clks=%0d — clamping to 1",sim_time_ns,clk_period_ns,clks))
      clks=1;
    end
  ...
  task run_phase(uvm_phase phase);
    ...
    wait_time_budget();
    env.scb.report();
    `uvm_info("TEST",$sformatf("FINAL FUNCTIONAL COVERAGE = %0.2f%%",env.cov.get_cov()),UVM_LOW)
    `uvm_info("TEST",$sformatf("MASTER_TX = %0.2f%%",env.cov.spi_cg.MASTER_TX.get_coverage()),UVM_LOW)
    `uvm_info("TEST",$sformatf("SLAVE_TX  = %0.2f%%",env.cov.spi_cg.SLAVE_TX.get_coverage()),UVM_LOW)
    `uvm_info("TEST",$sformatf("CORNERS   = %0.2f%%",env.cov.spi_cg.CORNERS.get_coverage()),UVM_LOW)
    `uvm_info("TEST",$sformatf("TX_CROSS  = %0.2f%%",env.cov.spi_cg.TX_CROSS.get_coverage()),UVM_LOW)
