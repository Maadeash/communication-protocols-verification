// ============================================================================
// i2c_pkg.sv
// Single-file UVM package for the I2C VIP. Must be compiled AFTER i2c_if.sv
// (driver/monitor use "virtual i2c_if.DRV" / "virtual i2c_if.MON" handles)
// and BEFORE tb_top.sv.
// ============================================================================
package i2c_pkg;
  ...
  // --------------------------------------------------------------------
  // i2c_txn — sequence item
  // --------------------------------------------------------------------
  class i2c_txn extends uvm_sequence_item;
    rand bit [6:0] addr;
    rand bit       rw;         // 0 = write, 1 = read
    rand bit [7:0] data;       // data driven by master on a write
    bit     [7:0] read_data;   // data captured on a read (dout)

    // Slave only recognizes SLAVE_ADDR (7'b0101010) — same assumption the
    // original testbench's generator made implicitly.
  ...
  // Mirrors the original generator: alternating write/read, random data.
  ...
  // Corner-case stimulus: 00, FF, 55, AA on write, followed by a read-back.
  ...
  class i2c_driver extends uvm_driver#(i2c_txn);
    ...
    task run_phase(uvm_phase phase);
      ...
      vif.drv_cb.rw     <= 1'b0;
      vif.drv_cb.w_data <= '0;
      // Do not start pulling items until reset has been released — without
      // this, sequence stimulus can race ahead of tb_top's reset-hold
      // window and pulse "enable" while rst==1, which the master's FSM
      // silently ignores (state is forced to idle on posedge rst).
  ...
  class i2c_agent extends uvm_agent;
    ...
    function void build_phase(uvm_phase phase);
      super.build_phase(phase);
      // allow a test to override active/passive via config_db; defaults to
      // the uvm_agent base class default (UVM_ACTIVE) otherwise
  ...
  // --------------------------------------------------------------------
  // i2c_scoreboard
  // Fix vs. original TB: the original scoreboard forked a mon2scb.get()
  // loop that discarded every monitor transaction and instead compared a
  // write's data against the *same driver's own* read_data — i.e. it was
  // self-checking against itself, never against an independent observer.
  // Here the scoreboard's imp is fed by the monitor's analysis port only,
  // so the write-then-read echo check is against an independently
  // sampled transaction stream.
  // --------------------------------------------------------------------
  class i2c_scoreboard extends uvm_scoreboard;
    ...
    function void report_phase(uvm_phase phase);
      `uvm_info("SCOREBOARD",
        $sformatf("PASS=%0d FAIL=%0d SKIP=%0d",pass_cnt,fail_cnt,skip_cnt),
        UVM_NONE)
    endfunction
  ...
  class i2c_cov_subscriber extends uvm_subscriber#(i2c_txn);
    ...
    function void report_phase(uvm_phase phase);
      `uvm_info("COVERAGE",
        $sformatf("FINAL FUNCTIONAL COVERAGE=%0.2f%%",cg.get_coverage()),
        UVM_NONE)
    endfunction
  ...
  class i2c_base_test extends uvm_test;
    ...
    function void start_of_simulation_phase(uvm_phase phase);
      ...
      // 34+ write/read pairs at ~3100ns each need more than the 9200ns
      // default watchdog; 300us covers well past the largest sequence used
      // by any test in this file.
      uvm_top.set_timeout(300us, 0);
    endfunction
  endclass : i2c_base_test

  // Reproduces the original TB's default stimulus: 5x write/read pairs.
  class i2c_random_test extends i2c_base_test;
    ...
    task run_phase(uvm_phase phase);
      ...
      #120000; // drain time for the last transaction's tail wait
      phase.drop_objection(this);
    endtask
  endclass : i2c_random_test

  // Corner-value data coverage: 00, FF, 55, AA.
  ...
  // Full regression: random write/read pairs followed by corner values.
